...
export default function SeeVisit() {
    ...
    const loadVisit = async () => {
        ...
        let joints = ['Ginocchio destro', 'Ginocchio sinistro', 'Gomito destro', 'Gomito sinistro', 'Caviglia destra', 'Caviglia sinistra']
        ...
        if (visit == null) {
            ...
            return (
                <div style={style.box}>
                    ...
                    <div style={{ display: 'flex', justifyContent: 'center', background: selectedJoint != null ? '#1e90ff' : '#2f4f4f', width: '100%', height: '5vh', alignItems: 'center', }}>
                        <h2 style={{ color: "white" }} >{selectedJoint != null ? 'Dettagli articolazione' : 'Dettagli visita'}</h2>
                    </div>

                    <div style={{ display: 'flex', width: '100%', justifyContent: 'center', }}>

                        <div style={{ overflow: 'scroll', display: 'flex', flexDirection: 'column', width: '25%', border: '1px solid lightgray', borderRadius: 15, }}>
                            <div style={{ width: '100%', textAlign: 'center', padding: '2vh', background: selectedJoint == null ? '#2f4f4f' : 'white', borderRadius: 15 }} >
                                <button onClick={() => setSelectedJoint(null)} className={selectedJoint != null ? "btn btn-lg btn-primary" : "btn btn-lg btn-light"}>Dettagli visita</button>
                            </div>

                            {visit.joints.map((item, index) => (
                                <div style={{ width: '100%', textAlign: 'center', padding: '2vh', background: selectedJoint == item.jointName ? '#1e90ff' : 'white', borderRadius: 15 }} >
                        ...
                        <div style={{ overflow: 'auto', width: '70%', textAlign: 'center', height: '75vh', border: '1px solid gray', padding: '1.5%', borderRadius: 15 }}>
                            <div>
                                {selectedJoint != null ? jointInfo() : visitInfo()}
                            </div>
                        </div>

                    </div>
                    <div>
                        <button className="btn btn-danger btn-lg" onClick={() => { navigate(-1) }} >Chiudi visualizzazione</button>
                    </div>
